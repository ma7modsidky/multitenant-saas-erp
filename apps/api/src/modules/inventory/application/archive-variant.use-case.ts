...
/**
 * ArchiveVariantUseCase — archives ONE variant of a product (INV-11).
 *
 * A variant with any movement history can never be hard-deleted — it is
 * archived (`is_active = false` + soft delete) and its ledger rows remain the
 * source of truth. The product itself stays active while other variants live.
 *
 * Emits `inventory.product.archived.v1` after commit with just this variant.
 */
@Injectable()
export class ArchiveVariantUseCase {
  constructor(
