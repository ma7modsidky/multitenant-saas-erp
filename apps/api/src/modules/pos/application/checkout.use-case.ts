...
export class CheckoutUseCase {
  ...
  async execute(input: CheckoutInput): Promise<{ saleId: string; receiptNumber: string }> {
    ...
    const committed = await this.txManager.run(async (tx) => {
      ...
      // POS-17: apply the org's default tax rate to lines that carry no
      // explicit rate (the client hardcodes 0) — ACC-11 via TAX_RATE_READ_PORT.
  ...
  private async resolveLineTaxes(lines: SaleLineInput[]): Promise<SaleLineInput[]> {
    // Resolve the default rate once and reuse it for every unrated line.
    let defaultRateBp = 0;
