...
describe('RemoveMemberUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new RemoveMemberUseCase(membershipRepo as never, txManager as never, jwtTokenService as never);
  });
  ...
  it('AUTHZ-1: rejects removing the last OWNER', async () => {
    ...
    await expect(runInContext(() => useCase.execute(input))).rejects.toMatchObject({
      code: 'LAST_OWNER_CANNOT_REMOVE',
    });

    expect(membershipRepo.update).not.toHaveBeenCalled();
  });

  it('AUTHZ-1: allows removing an OWNER when another owner exists', async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.OWNER }));
    ...
    expect(countCall[1]).toBe('role-member');
    expect(countCall[2]).toBe('tx');
    expect(membershipRepo.update).toHaveBeenCalledTimes(1);
  });

  it('AUTHZ-2: rejects an ADMIN removing an OWNER even when another owner exists', async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.OWNER }));
    membershipRepo.countByOrgIdAndRoleId.mockResolvedValue(2);

    // Ownership is OWNER-managed (AUTHZ-2) — an ADMIN cannot remove an OWNER
    // even though the last-owner guard would not fire.
    await expect(
      runInContext(() => useCase.execute({ ...input, currentUserRoleKey: SYSTEM_ROLES.ADMIN })),
    ).rejects.toMatchObject({
      code: 'ONLY_OWNER_CAN_REMOVE',
    });
  ...
  it('AUTHZ-1: does not count role holders for non-owner members (only the last OWNER is protected)', async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.MEMBER }));
  ...
  it('AUTHZ-7: soft-deletes the membership and records the actor', async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.MEMBER }));

    await runInContext(() => useCase.execute(input));

    const updateCall = membershipRepo.update.mock.calls[0] as [
      string,
      { status: string; deletedAt: Date; updatedBy: string },
      unknown,
    ];
    expect(updateCall[0]).toBe('membership-1');
    expect(updateCall[1].status).toBe('inactive');
    expect(updateCall[1].deletedAt).toBeInstanceOf(Date);
    expect(updateCall[1].updatedBy).toBe('user-1');
    expect(updateCall[2]).toBe('tx');
  });

  it("AUTHZ-5: revokes the removed member's sessions so no refresh re-issues old-org claims", async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.MEMBER, userId: 'user-2' }));

    await runInContext(() => useCase.execute(input));

    expect(jwtTokenService.revokeAllUserSessions).toHaveBeenCalledTimes(1);
    expect(jwtTokenService.revokeAllUserSessions).toHaveBeenCalledWith('user-2', 'MEMBER_REMOVED');
  });

  it('AUTHZ-5: does NOT revoke sessions when the removal is rejected', async () => {
    membershipRepo.findById.mockResolvedValue(makeMembership({ roleKey: SYSTEM_ROLES.OWNER }));
    membershipRepo.countByOrgIdAndRoleId.mockResolvedValue(1);
