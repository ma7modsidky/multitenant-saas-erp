...
export interface FxRatesRepository {
  ...
  getLatestRate(baseCurrency: string, quoteCurrency: string, tx?: TxOrDb): Promise<{ rate: string; validOn: string; source: string } | undefined>;

  /** Get a rate snapshot for a specific date (closest prior). */
  getRateForDate(baseCurrency: string, quoteCurrency: string, date: string, tx?: TxOrDb): Promise<{ rate: string; validOn: string; source: string } | undefined>;
  ...
  getLatestRatesForBase(baseCurrency: string, tx?: TxOrDb): Promise<Array<{ quoteCurrency: string; rate: string; validOn: string }>>;
}
