 ...
 *
 * The implementation lives in the shared lib (`@/lib/csv`) so every feature
 * exports spreadsheets with the same escaping rules; these aliases keep this
 * module's public API stable.
 */
import { buildCsv as buildSharedCsv } from '@/lib/csv';

export { downloadCsv } from '@/lib/csv';

/** Audit-flavored alias for the shared RFC-4180 CSV serializer. */
export function buildAuditCsv(headers: string[], rows: string[][]): string {
  return buildSharedCsv(headers, rows);
}
