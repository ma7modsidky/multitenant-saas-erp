...
export function TeamsView() {
  ...
  const teams = useQuery({ queryKey: ['teams', organizationId], queryFn: () => organizationId ? getTeams(organizationId) : Promise.resolve([]), enabled: !!organizationId });
  const members = useQuery({ queryKey: ['members', organizationId], queryFn: () => organizationId ? getMembers(organizationId) : Promise.resolve([]), enabled: !!organizationId });
  ...
  const saveMutation = useMutation({
    mutationFn: (state: TeamFormState) => {
      ...
      return state.id ? updateTeam(organizationId as string, state.id, payload) : createTeam(organizationId as string, payload);
    },
    onSuccess: () => {
      setForm(null);
      setError(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: (teamId: string) => deleteTeam(organizationId as string, teamId),
    onSuccess: () => {
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              <table className="w-full min-w-[560px] text-sm">
                <thead>
                  <tr className="border-b bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                    <th scope="col" className="px-3 py-2.5 text-start font-medium">{t('contacts.tableName')}</th>
                    <th scope="col" className="px-3 py-2.5 text-start font-medium">{t('teamsUI.leader')}</th>
                    <th scope="col" className="px-3 py-2.5 text-start font-medium">{t('teamsUI.members')}</th>
                    <th scope="col" className="px-3 py-2.5 text-start font-medium">{t('teamsUI.actions')}</th>
                  </tr>
                </thead>
                <tbody>
                  {(teams.data ?? []).map((team) => (
                    <tr key={team.id} className="border-b transition-colors last:border-0 hover:bg-accent/40">
                      <td className="px-3 py-2.5 font-medium" dir="auto">{team.name}</td>
                      <td className="px-3 py-2.5 text-muted-foreground">{memberName(team.leaderUserId)}</td>
                      <td className="px-3 py-2.5"><Badge variant="secondary">{team.memberCount}</Badge></td>
      ...
      {form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <button type="button" aria-hidden tabIndex={-1} className="absolute inset-0 cursor-default bg-black/50" onClick={() => setForm(null)} />
          <div className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border bg-card p-5 shadow-lg animate-fade-in">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="team-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} dir="auto" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="team-description">{t('teamsUI.description')}</Label>
                <Input id="team-description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} dir="auto" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label>{t('teamsUI.members')} ({form.memberUserIds.length})</Label>
                <div className="max-h-48 space-y-1 overflow-y-auto rounded-lg border p-2">
                  {activeMembers.map((m) => (
                    <label key={m.userId} className="flex items-center gap-2 rounded px-1 py-1 text-sm hover:bg-accent/40">
                      <input
                        type="checkbox"
              ...
              {saveMutation.isError && (
                <p role="alert" className="text-sm text-destructive">{t('errors.unknown')}</p>
              )}
