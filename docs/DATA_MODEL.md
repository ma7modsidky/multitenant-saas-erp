...
| `crm_pipelines`             | Deal pipelines                     | `name_i18n`, `is_default`, `owner_team_id` (CRM-17 team scoping; NULL = org-wide; bare uuid, no cross-prefix FK)                                                                                                                                        |
...
| `acc_tax_rates`           | Org tax rates                 | `code`, `name_i18n`, `rate_bp int`, `type` (`standard`\|`reduced`\|`zero`\|`exempt`), `tax_basis` (`exclusive`\|`inclusive`), `coa_account_id` → `acc_accounts.id` (where the tax posts — 2100 VAT payable / 2200 VAT receivable), `is_default bool` (partial unique index on `is_default`, ACC-11), `effective_from date`, `is_active` — `UNIQUE (organization_id, code)`                                                                                                                                                                                                                                                                                                                                             |
...
| `acc_invoices`            | AR invoices                   | `invoice_number`, `customer_contact_id` / `customer_company_id` (CRM ids, **no FK**), `customer_name_snapshot`, `customer_tax_id_snapshot`, `seller_tax_id`, `status` (`draft`\|`issued`\|`partially_paid`\|`paid`\|`overdue`\|`void`), `invoice_date`, `due_date`, `currency`, `exchange_rate`, `base_total_amount_minor`, `subtotal_*`, `discount_*`, `tax_*`, `total_*`, `paid_amount_minor` / `credited_amount_minor` (running projections of allocations + issued credit notes — ledger tables remain the source of truth, ACC-9/ACC-10), `locale`, `source_type` (`manual`\|`pos_sale`), `source_id`, `idempotency_key`, `e_invoice_uuid`, `e_invoice_hash`, `e_invoice_irn`, `e_invoice_qr`, `e_invoice_status` |
| `acc_invoice_lines`       | Invoice lines                 | `invoice_id`, `variant_id` (Inventory id, no FK), `item_name_snapshot`, `description`, `quantity numeric(18,4)`, `unit_price_*`, `discount_*`, `tax_rate_id`, `tax_rate_bp_snapshot`, `tax_type_snapshot`, `tax_basis_snapshot` (`exclusive`\|`inclusive`, ACC-11), `tax_*`, `line_total_*`, `is_goods bool`                                                                                                                                                                                                                                                                                                                                                                                                           |
| `acc_payments`            | AR receipts                   | `method` (`cash`\|`bank_transfer`\|`card`\|`cheque`\|`other`), `receipt_number` (`REC-xxxxx`, gap-free per org — ACC-9), `amount_*`, `currency`, `received_at`, `reference`, `idempotency_key` — `UNIQUE (organization_id, receipt_number)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `acc_credit_notes`        | AR credit notes (reversals)   | `invoice_id`, `credit_note_number`, `invoice_number` (snapshot of the reversed invoice — reproducibility, ACC-10), `status` (`draft`\|`issued`\|`void`), `reason_code`, `amount_*`, `currency`, `issued_at` — `UNIQUE (organization_id, credit_note_number)`                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `acc_org_settings`        | Per-org accounting settings   | `fiscal_year_start date`, `tax_registration_number`, `e_invoice_provider` (`none`\|`zatca`\|`eta`), `features jsonb` (`advanced_coa`, `e_invoicing`), `next_entry_number` / `next_invoice_number` / `next_credit_note_number` / `next_receipt_number` (per-org counters for gap-free numbers, ACC-3/ACC-10/ACC-9) — `UNIQUE (organization_id)`                                                                                                                                                                                                                                                                                                                                                                         |
...
| `pur_supplier_returns`      | Supplier returns / debit notes     | `number`, `supplier_id`, `bill_id`, `grn_line_id`, `reason_code`, `status` (`draft`\|`approved`\|`void`), `amount_*` (net), `tax_*`, `total_*` (gross = net + tax, PUR-11), `supplier_tax_id_snapshot`, `currency`, `returned_at` — `UNIQUE (organization_id, number)`                                                             |
| `pur_supplier_return_lines` | Returned lines                     | `return_id`, `variant_id`, `quantity`, `unit_cost_*`, `tax_rate_bp_snapshot`, `tax_*` — stock removed via `INVENTORY_MOVEMENT_PORT.returnToSupplier` (PUR-11)                                                                                                                                                                      |
