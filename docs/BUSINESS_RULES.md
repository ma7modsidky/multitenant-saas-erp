...
| BILL-2  | Each module may be trialled **once per organization**, for 14 days, without a payment method. A second trial of the same module is rejected with `TRIAL_ALREADY_USED`. Enforcement uses the permanent `trial_started_at` stamp: once set — even after natural expiry, an admin stop, or disable → re-enable — a fresh trial is rejected. Only an admin extension (`PLT-8`) or a direct paid grant (`skipTrial`/enable-now) can restore access. |
...
| PLT-8 | Platform admins control a per-org entitlement lifecycle beyond enable/disable: **extend trial** (`trial/extend` — pushes `trial_ends_at` forward from `max(now, current end)` for a `trialing`/`expired` module, reviving it to `trialing`), **stop trial** (`trial/stop` — `trialing → expired`, BILL-3 read-only grace; the BILL-2 stamp is preserved so the org cannot restart it), **suspend** (`active → suspended`, revokes a paid module while the Stripe item is kept), **activate** (`suspended/past_due/expired → active`). The state machine forbids `past_due → disabled` and `suspended → disabled` (activate first). All run inside `runWithOrg` (PLT-3) and are audited (PLT-4/BILL-13). Suspend/activate are local overrides: the BILL-4 reconciler still treats Stripe as the commercial authority, so a durable suspension must pair with a Stripe-side change. The admin never resets a trial — fresh trials are blocked for any entitlement whose `trial_started_at` is set (BILL-2). | `AdjustEntitlementUseCase` + state machine |
