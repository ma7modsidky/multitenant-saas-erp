...
This document is **the law**. Every rule here is an invariant that must be enforced in the domain layer and covered by at least one automated test. A rule that exists only in code is undiscoverable; a rule that exists only here is unenforced. Both must be true.

**Rule ID format:** `<AREA>-<n>`. Reference the ID in test names, e.g. `it('POS-12: rejects closing a shift with unsettled sales')`.
...
- Test names include the rule id: `it('INV-6: rejects an online sale exceeding available stock', ...)`.
- A CI report lists rule ids with no matching test and fails the build if a rule marked **critical** (all of TEN-*, AUTH-*, CUR-*, BILL-4, INV-1, INV-2, POS-26) is uncovered.
- Adding a rule here without a test in the same PR is not allowed.
- Changing behaviour that contradicts a rule requires updating this document **in the same PR** as the code.

---

## 13. Related documents

[PRD.md](./PRD.md) · [TECH_STACK.md](./TECH_STACK.md) · [ARCHITECTURE.md](./ARCHITECTURE.md) · [MODULE_GUIDE.md](./MODULE_GUIDE.md) · [DATA_MODEL.md](./DATA_MODEL.md) · [CODING_STANDARDS.md](./CODING_STANDARDS.md) · [TESTING.md](./TESTING.md)
